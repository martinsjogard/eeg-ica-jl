function msfun_meg_ica(raw::Dict, data::Array, extdata=nothing, cfg=Dict())
    ...
    if N != 306
        println("msfun_meg_ica - WARNING: Data may not come from Neuromag Elekta MEG system")
    end

    println("msfun_meg_ica - Preparing data for ICA...")

    if epoching
        println("msfun_meg_ica - Baseline correcting and concatenating epochs...")
    ...
    if get(cfg, "corranalysis", false)
        println("msfun_meg_ica - Preparing external signals for correlation analysis...")
        if epoching
            println("msfun_meg_ica -       baseline correcting and concatenating epochs...")
            X = zeros(S, K*T)
            for k in 1:K
    ...
    if epoching
        println("msfun_meg_ica - Restoring epochs in IC time courses...")
