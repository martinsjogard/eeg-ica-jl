...
function msfun_ica_estimate_eeg(sig::Matrix{Float64}, cfg::Dict=Dict())
    N, T = size(sig)
    if ndims(sig) != 2
    ...
    println("msfun_ica_estimate_eeg - Normalizing data units...")
    if normalize === nothing
        println("msfun_ica_estimate_eeg -       using temporal standard deviation of data...")
    ...
    println("msfun_ica_estimate_eeg - FASTICA running ...")
    ...
    println("msfun_ica_estimate_eeg - Restoring data to original data units ...")
    A_scaled = A .* normalize
    W_scaled = W ./ normalize'

    println("msfun_ica_estimate_eeg - Done.")
